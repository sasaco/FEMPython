...
ColorBar(canvasId):
  ...
  self.rect=:x:int(self.canvas.width/5),		# カラーバーの描画領域
      	     y:int(self.canvas.height/10),
      	     width:int(self.canvas.width*3/10),
      	     height:int(self.canvas.height*4/5)}
  self.gradient=self.context.createLinearGradient
    (self.rect.x,self.rect.y+self.rect.height,self.rect.x,self.rect.y)
...
# 数値表示位置
  self.maxPos=:x:xt,y:int(self.rect.y-CBAR_FONT_SIZE/2)}
  self.minPos=:x:xt,y:self.maxPos.y+self.rect.height}
}

# コンター図を描画する
# minValue - コンター図データ最小値
# maxValue - コンター図データ最大値
ColorBar.prototype.draw(minValue,maxValue):
